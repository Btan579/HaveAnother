         ...
         return chai.request(app)
             ...
             .then(function (res) {
                 ...
                 res.body.beers.forEach(function (review) {
                     review.should.be.a('object');
                     review.should.include.keys('id', 'name', 'brewery', 'category', 'style', 'reviews');
                 });
                 resBeers = res.body.beers[0];

                 return Beer.findById(resBeers.id);
             })
             .then(beer => {
                 resBeers.id.should.equal(beer.id);
                 resBeers.name.should.equal(beer.name);
                 resBeers.brewery.should.equal(beer.brewery);
                 resBeers.haveAnother.should.equal(review.haveAnother);
                 resBeers.user.should.equal(review.user._id.toString());
             });
     });
