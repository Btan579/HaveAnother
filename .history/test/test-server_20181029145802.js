...
function seedUserData() {
    ...
    User.insertMany(users).save().then(results => {
...
function seedCategoriesData() {
    ...
    for (var i = 1; i < 10; i++) {
     ...
     Category.save(categories).then(results => {
        console.log(results);
...
function seedStylesData () {
    ...
    Style.save(styles).then(results => {
...
function seedReviewData() {
    ...
    for (let j = 0; j < state.beerDocs.length; j++) {
        for (let i = 0; i < 10; i++) {
            createReviewSeed(state.beerDocs[j]);
        }
    } 
}
